...
async def get_starboard_msg(msg_id):
    return await standby.pg_pool.fetchrow(
        f"SELECT * FROM starboard WHERE msg_id = {msg_id};"
    )


async def get_starboard_sbid(msg_id):
    return await standby.pg_pool.fetchrow(
...
async def starboard_handler(payload):  # noqa: C901, PLR0912, PLR0915
    if isinstance(payload, RawReactionActionEvent) and payload.emoji.name == "⭐":
        logger.info("Star react added")
        chnl = standby.bot.get_channel(payload.channel_id)
        msg = await chnl.fetch_message(payload.message_id)
        await db.get_or_insert_usr(standby.bot, msg.author.id, payload.guild_id)
        stars = 0
        ...
        msg = await get_starboard_msg(payload.message_id)
        if msg is not None:
            await starboard_lock.acquire()
            try:
                await msg.delete()
